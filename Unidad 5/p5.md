...
$\Delta (id_C) = (id_C, id_C) = id_{(C , C)} = id_{\Delta (C)}$

$\Delta (f \circ g) = (f \circ g, f \circ g) = (f, f) \circ (g, g) = \Delta f \circ \Delta g$
...
- $id_C^{-1}$
- = def
- $\lambda X. \{ a \in C : id_C(a) \in X \}$ ($X \sub C$)
- = def id_C
